# Mohammad Reza Servati

Applied Cryptography, Security Protocols, and Privacy Researcher

## About Me
I am a security researcher specializing in applied cryptography and authentication protocols, with a particular focus on secure and lightweight authentication and key agreement schemes for IoT and resource-constrained systems.

I hold an M.Sc. in Computer Engineering, where my thesis was awarded the **Best M.Sc. Thesis in Cryptography (National Level)** by the Iranian Society of Cryptology. My research has resulted in multiple **Q1 journal publications**, including works published in *Elsevier Computer Networks*, *Pervasive and Mobile Computing*, and *IEEE Access*.  
My work has received **100+ citations**, and I actively serve as a reviewer for several high-impact journals.

## Research Interests
- Applied Cryptography and Security Protocols  
- Authentication and Key Agreement  
- IoT and Lightweight Cryptography  
- Formal Verification of Security Protocols  
- Privacy-Preserving and Secure Systems  

## Publications & Research
- Multiple Q1 journal publications with 100+ citations  
- Active reviewer for high-impact journals (Elsevier, Springer Nature, Nature Portfolio)  
- Google Scholar: https://scholar.google.com/citations?user=Cg-bbqkAAAAJ

## Reproducible Research
This GitHub hosts reproducible research artifacts, including:
- Implementations of authentication and key agreement protocols  
- Cryptanalysis and attack simulations  
- Formal verification artifacts (ProVerif, Scyther, AVISPA)  
- Experimental and performance evaluation scripts  

Each repository is documented to support reproducibility and further research.

## Contact
- Email: mohammadreza.servati75@gmail.com  
- GitHub: https://github.com/mohammadrezaservati
